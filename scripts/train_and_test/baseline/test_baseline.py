...
MODEL_PATH = Path("models/baseline_pitch_type/final_model.keras")
...
def analyze_per_class_performance(y_true, y_pred, pitch_lookup):
    ...
    # Show distribution of actual pitch types in test set
    print("\nPitch Type Distribution in Test Set:")
    print("-" * 50)
...
def main():
    print("=" * 60)
    print("Testing Baseline Pitch Sequence Model")
    print("=" * 60)
    ...
    if not TEST_DIR.exists() or not list(TEST_DIR.glob("*.parquet")):
        ...
        print("Please create test data using make_gold.py")
        return
    ...
    # Load test data
    print("\nLoading test data...")
    test_df = load_parquet_files(TEST_DIR)
    ...
    test_ds = df_to_tf_dataset(test_df, target_col, BATCH_SIZE)
    ...
    y_true = test_df[target_col].values.astype("int32")
    ...
    # Analyze by context
    analyze_predictions_by_context(test_df, y_true, y_pred, y_pred_probs)
    ...
    for idx in sample_indices:
        ...
        if PREDICT_TARGET == "pitch_type":
            ...
            print(f"\nSample {idx}:")
            print(f"  Context: {int(test_df.iloc[idx]['balls_before_pitch'])}-{int(test_df.iloc[idx]['strikes_before_pitch'])} count, "
                  f"Inning {int(test_df.iloc[idx]['inning'])}, Pitch #{int(test_df.iloc[idx]['pitch_num_in_pa'])}")
